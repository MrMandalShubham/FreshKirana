...
| 3 | P3.4 | COD risk & confirmation | ⏳ | | `3a52301` + `e713b79` · CI green, deployed · 603 tests · four bands, WhatsApp + OTP, thresholds in a table (no deploy), full audit log · sweep verified executing on GCP · **fixed two shipped defects: prepaid orders never reached the store, and three scheduled jobs were running months-old images** |
...
| 2026-08-20 | **P3.4** | **The COD confirmation has no customer screen.** The API is complete — status, confirm, decline and OTP verification are all reachable — but the PWA has no block on the order page for "we need you to confirm this", so today the flow works over WhatsApp and through the API only. | **Before pilot**, with the P3.3 recovery screens as the template |
