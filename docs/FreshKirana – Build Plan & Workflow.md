...
| 4 | P4.1 | Substitutions **+ a minimal picker screen** | ⏳ | | `d062318` · `a936ffc` · CI green, deployed · 681 tests · three preferences, ranked options, 10-minute window with refund fallback, price-delta rules · sweep verified on GCP · **fixed four shipped defects: the ranker ignored §1.7.2's safety rules, one template carried two messages, five sweeps compared two different clocks, and three could starve their own backlog** |
...
| 5 | **P5.2a** | **`vendor` → `branch` — the rename** | ⏳ | | `npm run verify` green · **800 tests** · pure rename, no behaviour change · schema, table, 9 `vendor_id` columns and 6 indexes renamed in migration 0026 — hand-written, because drizzle-kit asks "created or renamed?" interactively and a wrong answer drops the data · drizzle confirms **no drift** against the TS schema · HTTP routes unified to `branch/` · **fixed a defect the rename introduced: four DB constraint names are matched as strings for error messages, so renaming them silently killed every friendly message** — new `branch.e2e.spec.ts` covers it, verified by reintroducing the bug |
| 5 | P5.2b | `branch` — the semantics | ☐ | | Delete approval/suspension/onboarding, add `is_hub`, force QUANTITY inventory mode. Split out of P5.2 so the rename stayed mechanically verifiable |
...
| 2026-08-22 | **P4.3** | ⚠️ **Batches became rows.** P1.2 put `batch_no` and `expiry_date` on the *offer*, which says a store holds exactly one lot of anything | Real shops hold two — Monday's crate and Thursday's, expiring days apart — and §1.7.3's recall names *a batch*, not a product. With one lot per offer, FEFO has nothing to order and a recall has to withdraw every packet of a brand. The offer's own columns are left alone: they describe the lot currently being sold from, which is what search and the product page read |
| 2026-08-22 | **P4.3** | **The order line records which lot it was picked from** | Without that column, "which customers received the contaminated lot?" has no answer — and FSSAI compliance is not a report anybody can assemble afterwards from prices and dates. It is the entire argument for batches existing |
| 2026-08-22 | P4.3 | Shelf life is judged as a **percentage of total life**, never a number of days | Two days left means opposite things for paneer and for atta. §1.7.3 states it as a percentage for that reason, and a fixed day count would either block half the dry goods or wave through milk that sours tomorrow |
| 2026-08-22 | P4.3 | Delisting a batch **does not delist the offer** | A store with a fresh crate and a stale one should keep selling. The offer only goes unavailable when nothing sellable is left, which is checked after every delisting |
| 2026-08-22 | P4.3 | A batch with **no expiry sorts last**, not first | Treating a null expiry as zero would float every non-perishable to the top of the picking list, which is the exact opposite of what FEFO is for |
| 2026-08-22 | **P4.3** | **A recall blocks sale before it does anything else** — before the recall row is written, before anyone is told | Every minute a recalled lot stays sellable is another customer receiving it. Notification is a separate call for the same reason: blocking must not wait on a messaging provider, and somebody watching a slow request is somebody who presses the button again |
| 2026-08-22 | P4.3 | Recall notifications go to **delivered orders first** | Somebody with it in their kitchen needs to know now. Somebody whose order has not been picked is not at risk — their batch was blocked from being picked in step one |
...
| 2026-08-22 | **P4.2** | **A weight outside the band is not recorded until the customer agrees** | Recording it and asking afterwards leaves the order briefly carrying a price nobody agreed to — and anything reading it in that window, an invoice or a rider's collectable, would act on it. The order keeps the price the customer accepted until they accept a different one |
| 2026-08-22 | P4.2 | Differences below ₹5 are **absorbed, but only in the customer's favour** | A gateway refund costs a fee, takes days, and makes a bank line somebody reconciles — for less than one tomato. Money the customer *owes* is never written off against them, because that is charging somebody silently, which is the opposite of the rule this part exists to enforce |
| 2026-08-22 | P4.2 | The collectable is **rounded down on the half-rupee** | A rider and a customer settling 47 paise at a doorstep is a fiction; there is no coin for it. Rounding towards the customer is the same asymmetry as absorbing a small refund |
| 2026-08-22 | ⚠️ **P4.2** | **I read `quantity` on a loose line as a pack count**, which priced a kilo of tomatoes as a gram | P2.1 had already settled it — a measured line's quantity *is* the amount, 1500 with uom `G` being 1.5 kg — and I introduced a second reading rather than using the `quantityModeFor` contract that existed. Caught by the first test that checked an actual number. A second interpretation of a settled rule is worse than no rule, because both look right in isolation |
| 2026-08-21 | **P4.1** | ⚠️ **Three sweeps had `LIMIT` with no `ORDER BY`.** All now take the oldest first | The refund sweep had 204 rows matching a limit of 200, so which two hundred came back was undefined — Postgres may return any slice. A backlog larger than the limit can therefore starve the same rows indefinitely, and those rows are people waiting on money they are owed. Refunds now order by `initiated_at`, COD confirmations and substitutions by `expires_at`. The two payment sweeps already had it right, which is why it was three of five rather than all |
| 2026-08-21 | **P4.1** | **A minimal picker screen, built early and expected to be deleted** | The vendor's interface is WhatsApp (§0.3) and the dashboard is P7.1 — that has not changed. What changed is that P4.1 and P4.2 are the first parts whose flow *starts* with the picker, so their confirmation tests were reachable only from a terminal, and a part that cannot be confirmed is a part reported as done on the strength of its own tests. Placed inside the existing PWA rather than as a fourth deployable surface: a new app means a Dockerfile, a Cloud Run service, a CI job and Terraform, for something P7.1 replaces. The routes are code-split, so a shopper never downloads them |
| 2026-08-21 | P4.1 | The picker **reports the empty shelf and cannot choose what follows** | The customer's preference decides, applied by the API. A picker who could choose would be deciding for somebody who already said what they wanted — and the screen would then be a second place where §1.7.2 is interpreted |
| 2026-08-21 | **P4.1** | ⚠️ **Five sweeps compared a database timestamp against the application's clock.** All now use `now()` in SQL | Every timestamp on those rows is written by Postgres, so comparing it to a `Date` from Node mixes two clocks — and on Cloud Run that is one clock per instance. Found the honest way: this machine drifted **2.7 seconds behind Cloud SQL**, and a payment created *by the database* was not yet "older than now" according to the *application*, so reconciliation reported nothing to do while an order sat unpaid. Payment reconciliation, payment expiry, COD confirmation expiry, substitution timeout and stale-refund detection were all affected. One clock removes the class |
| 2026-08-21 | P4.1 | The `now` parameters those sweeps took were **removed, not left ignored** | Once the comparison moved into SQL the argument did nothing, and a parameter a caller can pass that is silently discarded is worse than no parameter — it reads like a seam for testing and is a lie |
| 2026-08-21 | **P4.1** | ⚠️ **The substitute ranker would have sent meat in place of paneer.** P2.7 built it with category, unit and a half-to-double size band only | §1.7.2's other rules — never cross veg/non-veg, never swap loose for packaged, ±25% on size — were simply missing, and none of them is a *ranking* concern: a non-veg substitute is not a low-scoring match to be sorted below better ones, it is a harm, and in much of India a serious one. Now a hard refusal in `contracts`, shared by the ranker and the flow, returning *why* so a picker is not left guessing why the obvious swap is not offered |
| 2026-08-21 | **P4.1** | ⚠️ **`SUBSTITUTION_PENDING` fired a second customer message using the proposal's own template.** Removed from the status map | P2.6 mapped the status to `SUBSTITUTION_PROPOSE` because nothing else sent that message yet. Once the §1.7.2 flow did, one template name carried two different payloads — "here are three options" and "your order moved" — and §2.12 registers each template with the provider against a fixed variable list. In production exactly one of the two would render. The map's own comment already argued against it: tell people when something is expected of *them* |
| 2026-08-21 | P4.1 | A timeout falls back to **a refund, never to the saved preference** | Somebody who chose ASK_ME asked to be asked; reading their silence as "go ahead" is precisely what they opted out of. A refund is the only fallback that cannot deliver something unwanted, which is why §1.7.2 names it |
| 2026-08-21 | P4.1 | A dearer substitute is **offered at the original price**, not blocked | §1.7.2 says never charge more without explicit consent — so the substitution still happens, held at what the customer agreed to, and the difference goes through settlement (§2.11) rather than onto their bill. Charging somebody more for an item they did not choose, because a shop ran out, is indefensible however small the amount |
| 2026-08-21 | P4.1 | Consent is **a recorded fact**, not an inferred one | "Explicit consent" is only enforceable if somebody can point at it afterwards, so it is a column on the row rather than a conclusion drawn from the price |
| 2026-08-21 | P4.1 | Accepting is restricted to **the options that were shown** | The offers are snapshotted onto the row as JSON, and an arbitrary offer id is refused — otherwise this route would substitute in exactly the matches §1.7.2's rules had just refused, and an offer that changed price an hour later would rewrite what the customer was asked |
| 2026-08-21 | P4.1 | The picker is the **actor** on `PICKING → SUBSTITUTION_PENDING` | §2.6.1 gives that edge to the store, because that is who caused it — somebody in an aisle found an empty shelf. Passing `SYSTEM_ACTOR` had the transition refused and the refusal *logged*, so the customer was asked about an order still reading as "being packed". A swallowed error that leaves two surfaces disagreeing is worse than a loud one |
| 2026-08-21 | P4.1 | A re-tap is answered **before** the status is judged | Raising the first question moves the order out of PICKING, so a picker who taps the same line twice was being told the order "is not being picked". The second tap is the same intent as the first, and the honest answer is the question already open |
| 2026-08-21 | **P3.5** | ⚠️ **`order.payment_status` was never written after the order was created.** Now maintained on capture and on refund | The column has existed since P2.3 and §2.6.2 declares the whole payment axis, but nothing moved it off `PENDING` — so every prepaid order read as unpaid however thoroughly it had been paid. Invisible until something asks "how much of this money is ours?", which P3.5 does and P5's ledger will do far more sharply. Found because the refund logic returned zero for orders that were plainly paid |
...
| 2026-08-25 | **P5.2a** | ⚠️ **Local work runs against staging Cloud SQL through the Auth Proxy — not a local container.** README now says so | `db_public_ip` defaults true precisely so a developer machine can reach the real database (`infra/terraform/variables.tf`), and the P3.4 entry below already talks about round trips through the proxy. Neither was obvious from the README, whose Getting Started opened with `npm run db:up` — so a whole part was built and "verified" against a local container while staging sat 26 migrations behind. **The tell is an authentication failure:** a stray Postgres container answers on 5432 in the proxy’s place, and the two have different passwords |
| 2026-08-25 | **P5.2a** | `npm run db:reset` is a **Docker** command and can never reach the cloud | `docker compose down -v` destroys local volumes only. That is a real safety property worth keeping: the destructive script is the one that is structurally incapable of touching staging. It also means it silently does nothing useful while the proxy is the thing serving 5432 |
| 2026-08-25 | **P5.2a** | A schema rename on a shared database is **RENAME, never drop-and-recreate** | Staging held 1370 branches, 5327 orders and 6543 carts. drizzle-kit asks "created or renamed?" interactively and cannot be scripted; had it been answered "created", the generated migration would have built an empty table and dropped the populated one. Migration 0026 was hand-written for that reason, and every row count matched before and after |
...
| 2026-08-22 | ⚠️ **P4.3** | **Stock is counted in two places.** `vendor_offer.stock_on_hand` stays authoritative for reservations — P3.1 guards it with a single atomic statement that is correct and hard-won — while `offer_batch.remaining_quantity` is the traceability record. Nothing reconciles them, so a batch consumed without the offer counter moving (or the reverse) drifts silently. Deliberate for this part: moving stock into batches means rewriting P3.1's oversell guarantee, which is a bigger change than §1.7.3 asks for. | **P7.1**, with the real inventory screen — or sooner if drift shows up in a pilot |
| 2026-08-22 | **P4.3** | **Nothing attaches a batch to a packaged line.** The weighing route carries `offerBatchId`, so loose goods can be traced — but a sealed perishable is never weighed, and no other route records which crate it came from. So a recall on packaged stock finds the batches and no orders. | **P7.1**, whose picking list is where a scanner belongs |
| 2026-08-22 | P4.3 | **The recall report is JSON, not a document.** §1.7.3 says "regulator-ready", and what exists is an endpoint returning the affected orders. Turning that into something a person can file — letterhead, signature, FSSAI's expected fields — needs the invoice/document work. | **P5.2**, with tax and invoicing |
| 2026-08-22 | P4.3 | **Delisted stock is never marked down.** §1.7.3 offers "auto-delisted *or flagged for markdown*"; only the first is built, so short-dated stock becomes waste rather than a discount. | **After pilot**, once there is data on how much is being written off |
| 2026-08-22 | **P4.2** | **The tax invoice is not generated at all.** §1.7.1 requires it be built *after weighing, on the actual amount, never the estimate* — the weighing half is done and the invoice half does not exist yet. Nothing currently produces a GST invoice for any order. | **P5.2** (tax & invoicing), which is where the invoice belongs |
| 2026-08-22 | **P4.2** | **The customer is not told the new amount before dispatch.** §1.7.1 asks for the recomputed cash figure to be pushed to the rider *and* the customer over WhatsApp; the figure is recomputed and stored, and only the consent prompt is ever sent. A shopper currently learns the final number when the rider arrives. | **Before pilot**, alongside the §2.6.3 notification set |
| 2026-08-22 | P4.2 | **The tolerance band is never shown before purchase.** §1.7.1 wants add-to-cart and checkout to say "priced per kg, final price varies (±10%), you'll be charged for what's delivered". The string exists in the PWA dictionary from P1.5 and no screen renders the band itself. | **Before pilot** — it is the sentence that makes the whole flow fair |
| 2026-08-22 | P4.2 | **`stock_on_hand` is in the product's own unit**, so a loose item is stocked in grams — 50 kg of tomatoes is `50000`. True since P3.1 and never written down; discovered when a fixture stocked 500 and ordered a kilo. Nothing in the vendor UI says which unit it wants. | **P7.1**, with the real inventory screen |
| 2026-08-21 | **P4.1** | **A substitution is never proposed on the customer's screen.** The API is complete and the WhatsApp message carries the options, but the PWA has no block on the order page for "we could not find your atta — here are three things instead". The §1.7.2 flow therefore depends on a channel the customer may have muted. | **Before pilot**, with the P3.3 and P3.4 blocks as the template |
| ~~2026-08-21~~ | ~~P4.1~~ | ~~No picker screen.~~ **Closed the same day** (`a936ffc`): an order queue and a picking list with one-tap out-of-stock, inside the customer PWA and expected to be deleted by P7.1. | ✅ Done |
