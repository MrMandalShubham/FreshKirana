...
| 4 | P4.1 | Substitutions | ⏳ | | `d062318` · CI green, deployed · 681 tests · three preferences, ranked options, 10-minute window with refund fallback, price-delta rules · sweep verified on GCP · **fixed three shipped defects: the ranker ignored §1.7.2's safety rules, one template carried two messages, and five sweeps compared two different clocks** |
...
| 2026-08-21 | P4.1 | **No picker screen either.** Marking a line out of stock is one API call, and a vendor has nothing to tap — the whole flow starts with an endpoint only a developer can reach. §1.7.2's picker flow is the other half of this part. | **P6.2**, with the rider PWA, or sooner if a pilot store needs it |
